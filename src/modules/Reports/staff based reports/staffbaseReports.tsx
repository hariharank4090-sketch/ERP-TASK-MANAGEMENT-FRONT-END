...
import { fetchStaffBasedReport } from './staffReports.api';
import { getEmployeeDropdown } from '../../TodayPlan/todayplan.api';
...
const StaffBasedReports = () => {
    ...
    <Box sx={{ display: 'flex', gap: 2, alignItems: 'center', flexWrap: 'wrap' }}>
      <FormControl size="small" sx={{ minWidth: 200, flexGrow: 1 }}>
        <SearchableSelect
          ...
          allOptionValue=""
          options={Array.from(new Set(staffList.map((staff: any) => staff.Emp_Name || staff.Cost_Center_Name || staff.costCenterName || staff.name || "")))
            .filter(Boolean)
            .filter(name => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        // Fetch cost centers to build staffList
        const empRes = await getEmployeeDropdown();
        let empList: any[] = [];
  ...
  const renderAbstractView = () => {
    ...
    if (stockFilter === 'all' || stockFilter === 'data-with-0') {
      staffList.forEach((s: any) => {
        const name = String(s.Emp_Name || s.Cost_Center_Name || s.costCenterName || s.name || "").trim();
      ...
      <Box sx={{ mt: 2 }}>
        {isAdmin ? (
          <FilterableTable
            ...
            headerTitle="Staff Based Reports"
          ...
          <Paper sx={{ p: 2, display: 'flex', flexDirection: 'column' }}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2, alignItems: 'center', flexWrap: 'wrap', gap: 2 }}>
              <Typography variant="h6" fontWeight="bold">Staff Based Reports</Typography>
              {headerActionsContent}
            </Box>
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, overflowY: 'auto', maxHeight: 'calc(100vh - 260px)', pr: 1 }}>
              {dataArray.filter((row: any) => isAdmin || !row.isTotal).map((row: any, i: number) => (
                <Card key={i} sx={{ backgroundColor: row.isTotal ? '#f5f5f5' : '#fff', border: '1px solid #e0e0e0', boxShadow: 1, maxHeight: 'none !important', overflow: 'visible !important' }}>
                  <CardContent sx={{ pb: "16px !important", maxHeight: 'none !important', overflow: 'visible !important' }}>
                    <Typography variant="subtitle1" fontWeight="bold">
                      {row.isTotal ? "Grand Total" : `S.No: ${row.sNo} - ${row.staffName}`}
                    </Typography>
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1, mt: 1 }}>
                       {dates.map((date: string) => (
                         <Box key={date} sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #f0f0f0', pb: 0.5 }}>
                           <Typography variant="body2" color="textSecondary">{date}:</Typography>
                           <Typography variant="body2" fontWeight="medium">{row[date]}</Typography>
                         </Box>
                       ))}
                       <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #f0f0f0', pb: 0.5 }}>
                         <Typography variant="body2" color="textSecondary">Total:</Typography>
                         <Typography variant="body2" fontWeight="bold">{row.total}</Typography>
                       </Box>
                    </Box>
                  </CardContent>
                </Card>
  ...
  const renderExpandedView = () => {
    ...
    data.forEach((row: any) => {
      ...
      categoryFields.forEach((field) => {
        ...
        const duplicateKey = `${row.Invoice_no || row.InvoiceId || 'inv'}_${row.Trans_Id || row.Trip_Id || ''}_${field}_${staff}`;
                ...
                <Card key={i} sx={{ backgroundColor: row.isTotal ? '#f5f5f5' : '#fff', border: '1px solid #e0e0e0', boxShadow: 1, maxHeight: 'none !important', overflow: 'visible !important' }}>
                  <CardContent sx={{ pb: "16px !important", maxHeight: 'none !important', overflow: 'visible !important' }}>
                    <Typography variant="subtitle1" fontWeight="bold" mb={1}>
                      {row.isTotal ? "Grand Total" : `S.No: ${row.sNo} - ${row.staffName}${isAdmin ? ` (Godown: ${row.godownName})` : ''}`}
                    </Typography>
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1, mt: 1 }}>
                       {categoryFields.map((f: string) => (
                         <Box key={f} sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #f0f0f0', pb: 0.5 }}>
                           <Typography variant="body2" color="textSecondary">
                             {f.replace(/_/g, " ")}:
                           </Typography>
                           <Typography variant="body2" fontWeight="medium">{row[f]}</Typography>
                         </Box>
                       ))}
                       <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #f0f0f0', pb: 0.5 }}>
                         <Typography variant="body2" color="textSecondary">Total Qty:</Typography>
                         <Typography variant="body2" fontWeight="bold">{row.Total}</Typography>
                       </Box>
                    </Box>
                  </CardContent>
...
export default StaffBasedReports;
