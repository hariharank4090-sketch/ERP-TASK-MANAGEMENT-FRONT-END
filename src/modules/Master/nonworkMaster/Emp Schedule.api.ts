  ...
  try {
    ...
    if (res && res.success) {
      ...
      if (Array.isArray(data)) {
        ...
        (data as any).data.forEach((item: any) => {
          const id = Number(item.Task_Type_Id || item.Type_Id || item.value);
          const name = item.Task_Type || item.Type_Name || item.label;
          if (id && name) {
            taskTypeMap[id] = name;
      ...
      const transformedData = responseData.map(item => {
        ...
        return {
          ...
          taskDates: item.taskDates?.map((td: any) => ({
            ...
            taskWorkDate: td.taskWorkDate || td.Task_Work_Date,
            taskStartTime: td.taskStartTime || td.Task_Start_Time,
            taskEndTime: td.taskEndTime || td.Task_End_Time,
          ...
          planDetails: item.planDetails?.map((pd: any) => ({
            planMonth: pd.planMonth || pd.Plan_Month,
            planDay: pd.planDay || pd.Plan_Day
          })) || []
        };
      });
    ...
    if (res && res.success) {
      ...
      if (Array.isArray(data)) {
        return data.map(item => ({
          value: Number((item as any).Project_Id || (item as any).value || (item as any).projectId),
          label: (item as any).Project_Name || (item as any).label || (item as any).projectName,
        }));
      } else if (data && typeof data === 'object' && 'data' in data && Array.isArray((data as any).data)) {
...
export const gettaskDropdown = async (
  ...
  try {
    ...
    if (res && res.success) {
      ...
      if (Array.isArray(data)) {
        return data.map(item => ({
          value: Number((item as any).Task_Id || (item as any).value || (item as any).taskId),
          label: (item as any).Task_Name || (item as any).label || (item as any).taskName,
        }));
      } else if (data && typeof data === 'object' && 'data' in data && Array.isArray((data as any).data)) {
        return (data as any).data.map((item: any) => ({
...
// Get task type dropdown with Task_Type_Id and Task_Type
export const gettaskTypeDropdown = async (
  ...
  try {
    ...
    if (res && res.success) {
      ...
      if (Array.isArray(data)) {
        return data.map(item => ({
          Task_Type_Id: Number((item as any).Type_Id || (item as any).Task_Type_Id || (item as any).value || (item as any).typeId),
          Task_Type: (item as any).Type_Name || (item as any).Task_Type || (item as any).label || (item as any).typeName || '',
        }));
      } else if (data && typeof data === 'object' && 'data' in data && Array.isArray((data as any).data)) {
        return (data as any).data.map((item: any) => ({
